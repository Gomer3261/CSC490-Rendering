...
float width = fbo_width; //texture width
float height = fbo_height; //texture height
...
float doMist()
{
  float zdepth = texture2D(fbo_depth, f_texcoord).x;
...
{
  if (f_texcoord.x<0.0||f_texcoord.y<0.0) return 1.0;
  return (2.0 * znear) / (zfar + znear - texture2D(fbo_depth, coord ).x * (zfar-znear));
}

float compareDepths(in float depth1, in float depth2,inout int far)
{
  float garea = 2.0; //gauss bell width
  float diff = (depth1 - depth2)*100.0; //depth difference (-100-0-100)
...
float calAO(float depth,float dw, float dh)
{
  ...
  float temp = 0.0;
  float temp2 = 0.0;
  float coordw = f_texcoord.x + dw*dd;
  float coordh = f_texcoord.y + dh*dd;
  float coordw2 = f_texcoord.x - dw*dd;
  float coordh2 = f_texcoord.y - dh*dd;
...
void main(void)
{
  ...
  vec3 luminance = vec3(lum, lum, lum)*lumInfluence;

  vec3 final = color*mix(vec3(ao),vec3(1.0),luminance*lumInfluence);

  //vec3 final = color*mix(vec3(ao), vec3(1.0), luminance);

  if (onlyAO)
  {
    final = mix(vec3(ao),vec3(1.0),luminance*lumInfluence); //ambient occlusion only
  }
